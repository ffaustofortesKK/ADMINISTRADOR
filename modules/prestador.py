...
# --- Funções auxiliares caso precise delas no módulo ---
...
def obter_video_fundo(token):
    try:
        response = requests.get(f"{FIREBASE_URL}/config/{token}/video_fundo.json")
...
def definir_video_fundo(token, url):
    try:
        requests.put(f"{FIREBASE_URL}/config/{token}/video_fundo.json", json=url)
    except Exception:
        pass


def listar_videos_pasta_clipes():
    # Retorne a sua lista de clipes do Cloudinary aqui se já tiver implementado
    return []


# --- Função Principal Completa (Exatamente a que enviou) ---
    ...
    try:
        ...
        if pedidos_ativos:
            for idx, p in enumerate(pedidos_ativos, start=1):
                ...
                badge_texto = (
                    "🎵 A TOCAR AGORA" if is_playing else f"⏳ Fila #{idx}"
                )
                ...
                with st.container():
                    ...
                    with col_acao1:
                        if not is_playing:
                            if st.button(
                                "▶️ Tocar Agora",
                                key=f"play_linha_{p.get('id')}",
                                use_container_width=True,
                            ):
                                terminar_todas_musicas_ativas(
                                    provider_token, pedidos
                                )
                                atualizar_estado_pedido(
                                    provider_token, p.get("id"), "aprovado"
                                )
        ...
        with st.form(key="form_video_fundo"):
            escolha_video = st.selectbox(
                "Pesquisar Vídeo Clipe", options=opcoes_labels, index=index_atual
            )
