...
def listar_videos_pasta_clipes():
    # Integração para buscar os clipes disponíveis (pode ajustar conforme a sua API/Cloudinary se necessário)
    try:
        res = requests.get(f"{FIREBASE_URL}/clipes.json", timeout=10)
...
# --- Função de Gestão da Fila (Fragmento Atualizável) ---
...
def renderizar_gestao_fila_prestador(provider_token):
    try:
        ...
        # Secção da Pasta de Vídeo Clipes / Fundo
        st.markdown("### 🎬 Gestão de Vídeo Clipe / Fundo da Tela")
        ...
        for clipe in lista_clipes_cloudinary:
            nome_clipe = clipe.get('nome', 'Clipe')
            url_clipe = clipe.get('url', '')
            label = f"📁 {nome_clipe}"
...
# --- Função Principal do Painel do Prestador ---
def show_provider_panel_custom(provider_token):
    ...
    # Tenta carregar informações do prestador
    try:
        from modules.admin import get_all_providers
    ...
    # Injeção de Estilos CSS e Imagem de Fundo do Painel
    st.markdown(f"""
    <style>
    .stApp {{
    ...
    # Secção de reforço se necessário
    ...
    # Chama a secção dinâmica da fila e dos vídeos de fundo
