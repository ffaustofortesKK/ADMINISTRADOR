...
def show_provider_panel_custom(provider_token):
    ...
    col_info_1, col_info_2 = st.columns([2.2, 1], gap="medium")
    with col_info_1:
        st.markdown(f"""
            <div class="card-link">
                <div class="link-title">🔗 LINK 1 DO CLIENTE (REGISTO DE MÚSICA)</div>
                <a href="{link_cliente_rel}" target="_blank" class="link-text">{link_cliente_absoluto}</a>
            </div>
            <div class="card-tv">
                <div class="link-title-tv" style="color:#e040fb !important;">📺 LINK DA TELA DE TV / REPRODUÇÃO</div>
        ...
        st.markdown(f"""
            <div style="display: flex; align-items: center; justify-content: flex-end; gap: 10px;">
                <div style="text-align: center;">
                    <div style="font-family: monospace; color: #fff; font-size: 10px; font-weight: bold; margin-bottom: 2px;">QR CODE CLIENTE</div>
                    <div class="qr-box">
                        <img src="{qr_url_cliente}" width="85" style="border-radius: 4px;" />
                    </div>
                </div>
            </div>
        """, unsafe_allow_html=True)
