    ...
    try:
            ...
            for p in pendentes:
                ...
                st.markdown(f"""
                    <div style="color: #ffffff; font-family: monospace; font-size: 15px; margin-bottom: 15px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
                        <b>{titulo_p}</b> <span style="color: #ffffff; font-size: 13px; font-weight: bold;">({cliente_p})</span>
                    </div>
                """, unsafe_allow_html=True)
                ...
                with col_center_btn:
                    if st.button("✅ Sim", key=f"conf_sim_{p.get('id')}", use_container_width=True):
                        terminar_todas_musicas_ativas(provider_token, pedidos, FIREBASE_URL)
                        atualizar_estado_pedido(provider_token, p.get('id'), 'aprovado', FIREBASE_URL)
        ...
        if pedidos_ativos:
            for idx, p in enumerate(pedidos_ativos, start=1):
                ...
                badge_texto = f"{idx}° {cliente_nome}" if cliente_nome != "Convidado" else f"{idx}° {titulo_musica}"
                
                with st.container():
                    col_info, col_acao = st.columns([3, 1])
                    with col_info:
                        st.markdown(f"""
                            <div style="background: rgba(0,0,0,0.95); border: 4px solid {cor_borda}; border-radius: 8px; padding: 10px 12px; margin-bottom: 8px; font-family: monospace;">
                                <div style="color: #ffffff; font-size: 16px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
                                    {badge_texto} - <span style="font-size: 14px; color: #FFC107;">{titulo_musica}</span>
                                </div>
                            </div>
                        """, unsafe_allow_html=True)
                    with col_acao:
        ...
        if tocando_agora:
            if st.button("🛑 Stop Geral (Limpar Tela)", key="stop_geral_btn", use_container_width=True):
                terminar_todas_musicas_ativas(provider_token, pedidos, FIREBASE_URL)
                definir_video_fundo(provider_token, "", FIREBASE_URL)
                st.warning("Reprodução parada e tela limpa com sucesso!")
                st.rerun()

        st.markdown("---")
        
        video_fundo_atual = obter_video_fundo(provider_token, FIREBASE_URL)
        ...
        with st.form(key="form_video_fundo"):
            st.markdown("<p style='font-family: monospace; font-size: 14px; font-weight: bold;'>Pesquisar Vídeo Clipe</p>", unsafe_allow_html=True)
            ...
            btn_salvar_fundo = st.form_submit_button("Carregar / Aplicar Clipe")
            if btn_salvar_fundo:
                valor_a_guardar = "" if escolha_video == "Nenhum (Ecrã Preto)" else mapa_url_por_label.get(escolha_video, "")
                definir_video_fundo(provider_token, valor_a_guardar, FIREBASE_URL)
...
def show_provider_panel_custom(provider_token, FIREBASE_URL, get_all_providers):
    ...
    classe_piscar = "animation: piscarRelogio 1s infinite;" if 0 < segundos_restantes <= 1800 else ""
    ...
    # Layout Principal em 2 Colunas (Esquerda: Dados e Links, Direita: Relógio e QR Code)
    ...
    with col_esq:
        st.markdown(f"""
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
                ...
                <div>
                    <h1 style="margin: 0; font-family: monospace; font-size: 20px; text-transform: uppercase;">PAINEL DO PRESTADOR: {nome_prestador}</h1>
                    <p style="margin: 2px 0 0 0; font-size: 12px; font-family: monospace;">TOKEN: <code style="background: #222; padding: 1px 4px; border-radius: 3px;">{provider_token}</code></p>
                </div>
            </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class="card-link">
                <div class="link-title">🔗 LINK DO eeeCLIENTE (REGISTO DE MÚSICA)</div>
    ...
    with col_dir:
        st.markdown(f"""
            <div style="background: rgba(255,193,7,0.15); border: 2px solid #FFC107; padding: 8px; border-radius: 8px; text-align: center; margin-bottom: 10px;">
                <div style="font-family: monospace; font-size: 10px; text-transform: uppercase;">TEMPO / PLANO ESCOLHIDO</div>
                <div style="font-family: monospace; color: #FFC107; font-size: 13px; {classe_piscar}">⏱️ {tempo_formatado} ({tempo_plano})</div>
            </div>
        """, unsafe_allow_html=True)

        st.markdown("<div style='font-family: monospace; font-size: 10px; font-weight: bold; text-align: center; margin-bottom: 2px;'>QR CODE CLIENTE</div>", unsafe_allow_html=True)
        st.markdown(f"""
            <div style="background: #000; border: 3px solid #FFC107; border-radius: 8px; padding: 4px; display: flex; justify-content: center;">
                <img src="{qr_url_cliente}" width="105" style="border-radius: 4px;" />
            </div>
        """, unsafe_allow_html=True)
