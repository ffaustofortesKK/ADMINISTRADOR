...
def approve_provider(token):
    try:
        # 1. Procurar e atualizar em todos os caminhos possíveis no Firebase
        encontrou = False
            ...
            if resp.status_code == 200 and resp.json():
                ...
                for key, val in dados.items():
                    if isinstance(val, dict) and (val.get("token") == token or key == token):
                        dados_prestador = val
                        # Atualizar para approved = 1 no nó correspondente
                        requests.patch(f"{FIREBASE_URL}/{node}/{key}.json", json={"approved": 1}, timeout=10)
                        encontrou = True
                        
        # 2. Garantir registo direto na tabela principal ativa
        if dados_prestador:
            ...
            # Fallback se não encontrar o objeto completo
...
def main():
    try:
        ...
        if token:
            ...
            # Se não achou no dataframe geral, faz busca direta no Firebase por segurança
            ...
            if prestador_encontrado:
                ...
                if status_aprov == 1:
                    ...
                    # Ecrã de espera com recarregamento automático (Polling a cada 4 segundos)
