...
   }}
    ...
    renderizar_gestao_fila_prestador(provider_token)@st.fragment(run_every=3)def renderizar_ecra_tv(provider_token):
    ...
    try:
        ...
        st.error(f"Erro de sincronização na TV: {e}")def show_client_screen():
    ...
    renderizar_ecra_tv(provider_token)def show_provider_panel_center(token):

    show_provider_panel_custom(token)def main():
    ...
    try:
        ...
        st.error(f"Ocorreu um erro ao carregar a aplicação: {e}")if __name__ == "__main__":
