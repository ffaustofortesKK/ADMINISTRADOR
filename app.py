...
def renderizar_gestao_fila_prestador(provider_token):
    try:
        ...
        if pedidos_ativos:
            ...
            st.markdown("""<div style="background-color: transparent; border: 4px solid #FFC107; border-radius: 8px; padding: 15px; color: #ffffff; width: 100%; font-family: monospace; font-size: 14px; margin-bottom: 20px; text-align: center; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
                    NENHUM PEDIDO NA LISTA NESTE MOMENTO.<br>À ESPERA DE NOVOS PEDIDOS...
                </div>
            """, unsafe_allow_html=True)
    except Exception as e:
        st.error(f"Erro na gestão da fila: {e}")
...
def renderizar_ecra_tv(provider_token):
    try:
        ...
        .block-container { background-color: #000000 !important; padding: 1rem 2rem !important; max-width: 100% !important; }
        </style>
        """
        ...
        if tocando_agora:
            ...
            cantor_atual = tocando_agora.get("cliente", "Convidado")

            video_html = f"""
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; background: #000000; padding: 0; width: 100%; position: relative;">
                <div id="countdown-screen" style="position: absolute; top: 0; left: 0; width: 100%; height: 600px; background: rgba(0,0,0,0.95); display: flex; align-items: center; justify-content: center; font-size: 140px; font-weight: bold; color: #FFC107; font-family: monospace; z-index: 999; text-shadow: 0 0 30px rgba(255,193,7,0.8);">
                    3
                </div>
                
                <div style="width: 100%; text-align: center; margin-bottom: 10px; background: rgba(0,0,0,0.9); border: 3px solid #FFC107; border-radius: 8px; padding: 8px;">
                    <span style="color: #FFC107; font-size: 20px; font-family: monospace; font-weight: bold;">🎤 A CANTAR AGORA:</span>
                    <span style="color: #ffffff; font-size: 22px; font-family: monospace; font-weight: bold; text-transform: uppercase;">{cantor_atual}</span>
                    <span style="color: #FFC107; font-size: 18px; font-family: monospace; margin-left: 15px;">🎵 {titulo_limpo}</span>
                </div>

                <div id="karaoke-container" style="display: none; width: 100%; text-align: center; position: relative;">
                    <video id="karaoke-player" width="100%" height="560px" autoplay playsinline controlslist="nodownload noremoteplayback" disablepictureinpicture style="object-fit: contain; background: black; border: 4px solid #FFC107; border-radius: 10px;">
                        <source src="{url_video}" type="video/mp4">
                        O seu navegador não suporta vídeo.
                    </video>
                    <div id="audio-warning" style="display: none; position: absolute; bottom: 20px; right: 20px; background: rgba(0,0,0,0.85); border: 2px solid #FFC107; padding: 8px 12px; border-radius: 6px; cursor: pointer;" onclick="unmuteVideo()">
                        <span style="color: #FFC107; font-size: 18px; font-family: monospace; font-weight: bold;">🔊 ATIVAR SOM</span>
                    </div>
                </div>
            </div>
...
def show_client_screen():
    ...
    st.markdown("""
    <style>
    .stApp { background-color: #000000; color: white; }</style>""", unsafe_allow_html=True)
