...
def show_client_page():
    ...
    .marquee-text {
        ...
        color: #FFC107;
        font-weight: bold;
        font-size: 18px; /* Texto da agenda maior */
    ...
    if not st.session_state.cliente_registado:
        st.markdown("<h2 style='color: #FFC107; font-size: 32px;'>🎤 Bem-vindo ao FF Karaoke</h2>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 20px;'>Insira o seu nome ou alcunha para começar:</p>", unsafe_allow_html=True)
    ...
    # Título de boas-vindas com letra maior (ex: 34px)
    st.markdown(f"<h1 style='color: #4CAF50; font-size: 34px; margin-bottom: 0;'>Benvindo, {cliente_nome}</h1>", unsafe_allow_html=True)
    ...
    if tem_pedido_ativo:
        # Texto da posição com letras grandes e cor amarela destacada
        st.markdown(f"""
            <div style="text-align: center; padding: 20px 10px; margin: 10px auto; max-width: 700px;">
                """ + (f'<div style="color: #FFFFFF; font-size: 24px; font-weight: bold; margin-bottom: 15px;">Encontra-se na <b style="color: #FFC107; font-size: 30px;">{posicao_fila}º</b> posição</div>' if posicao_fila else '') + """
                <div class="spinning-mic">🎤</div>
            </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("<p style='color: #4CAF50; font-size: 20px; font-weight: bold;'>✅ Já poderá enviar o seu pedido!</p>", unsafe_allow_html=True)

    if st.session_state.musica_selecionada:
        musica_atual = st.session_state.musica_selecionada
        st.markdown(f"""
            <div style="background: #161a23; padding: 25px; border-radius: 12px; border: 2px solid #4CAF50; text-align: center; margin: 10px 0 15px 0;">
                <h3 style="color: #4CAF50; margin-bottom: 10px; font-size: 24px;">Confirmação de Pedido</h3>
                <p style="font-size: 20px; font-weight: bold; margin-bottom: 15px;">Quer tocar <b style="color: #FFC107; font-size: 22px;">{musica_atual['titulo']}</b>?</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='color: #FFC107; font-size: 26px;'>🔍 Pesquisar Música</h3>", unsafe_allow_html=True)
    ...
    if pesquisa:
        ...
        if musicas_filtradas:
            st.markdown(f"<p style='font-size: 18px; color: #CCCCCC;'>Encontradas <b>{len(musicas_filtradas)}</b> músicas:</p>", unsafe_allow_html=True)
            ...
            with container_lista:
                for musica in musicas_filtradas:
                    cols = st.columns([4, 1])
                    with cols[0]:
                        st.markdown(f"<p style='font-size: 18px; margin: 5px 0;'>🎵 <b>{musica['titulo']}</b></p>", unsafe_allow_html=True)
                    with cols[1]:
    ...
    st.markdown("""
        <div style="background: linear-gradient(135deg, #1f1c2c, #928dab); padding: 30px; border-radius: 12px; text-align: center; color: white; margin-top: 30px;">
            <h3 style="margin-bottom: 12px; color: #FFC107; font-size: 24px;">Quer saber mais do serviço de Karaoke do Grupo FF, clica abaixo.</h3>
            <p style="font-size: 18px; margin: 10px 0;">📸 <b>Instagram:</b> <a href="https://instagram.com/ff.karaoke" target="_blank" style="color: #00d2ff; text-decoration: none;">ff.karaoke</a></p>
            <p style="font-size: 18px; margin: 10px 0;">📞 <b>Contacto para Eventos Privados:</b> 955099159</p>
            <p style="font-size: 18px; margin: 10px 0;">💬 <b>WhatsApp:</b> <a href="https://wa.me/244955099159" target="_blank" style="color: #25D366; text-decoration: none;">955099159</a></p>
        </div>
    """, unsafe_allow_html=True)
