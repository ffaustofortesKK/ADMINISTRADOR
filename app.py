...
def custom_show_register_page():
    ...
    if "token_pendente_prestador" in st.session_state and st.session_state["token_pendente_prestador"]:
        ...
        # Verificar no Firebase / DB se foi aprovado
        aprovado = False
        try:
        ...
        # Ecrã de espera com Logotipo, microfone a girar e rodapé solicitado
        st.markdown(f"""
            <style>
            @keyframes spinMic {{
...
def main():
    try:
        ...
        # ÁREA RESTRITA CENTRALIZADA (ADM) COM FUNDO PRETO E TODAS AS LETRAS EM NEGRITO
        st.markdown("""
            <style>
            .stApp {
