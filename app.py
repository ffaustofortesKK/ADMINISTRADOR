    ...
    try:
        ...
        if tocando_agora:
            if st.button("🛑 Stop Geral (Limpar Tela)", key="stop_geral_btn", use_container_width=True): 
                terminar_todas_musicas_ativas(provider_token, pedidos)
                defini_video_fundo_resp = definir_video_fundo(provider_token, "")
...
def show_provider_panel_custom(provider_token):
    ...
    # Conteúdo principal do painel integrando a gestão de fila de reprodução
    st.markdown(f"""
        <div class="panel-header">
            <div style="display: flex; align-items: center; gap: 15px;">
                <img src="{url_logotipo}" style="width: 65px; height: 65px; border-radius: 50%; border: 2px solid #FFC107; object-fit: cover;" />
                <div>
                    <h2 style="color: #FFC107; margin: 0; font-size: 24px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">PAINEL DO PRESTADOR</h2>
                    <p style="color: #ffffff; margin: 0; font-size: 14px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">Utilizador: <b>{nome_prestador.upper()}</b></p>
                </div>
            </div>
            <div style="text-align: right; background: #111; padding: 8px 15px; border: 2px solid #FFC107; border-radius: 6px;">
                <span style="font-size: 12px; color: #ccc; display: block; font-weight: bold;">TEMPO RESTANTE</span>
                <span style="font-size: 22px; font-weight: bold; color: #FFC107; font-family: monospace; {classe_piscar}">{tempo_formatado}</span>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # Executar a gestão de filas e conteúdos do prestador
