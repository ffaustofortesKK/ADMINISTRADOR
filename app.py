...
# ---------------------------------------------------------
# CONFIGURAÇÕES GLOBAIS E HELPERS (MOCK / PLACEHOLDERS)
# ---------------------------------------------------------
...
def show_register_page():
    st.title("Registo de Prestador")

def show_client_page():
    st.title("Registo de Cliente - FF Karaoke")
...
# ---------------------------------------------------------
# FUNÇÃO PRINCIPAL DE RENDERIZAÇÃO DA TV / PAINEL DO PRESTADOR
# ---------------------------------------------------------
def renderizar_ecra_tv(provider_token, tocando_agora=None, pedidos_ativos=None):
    if pedidos_ativos is None:
        pedidos_ativos = []
        ...
        if tocando_agora:
            ...
            with col_esq:
                ...
                if not pedidos_ativos:
                    # AUMENTO DO TAMANHO DAS LETRAS E NEGRITO PARA OS LINKS DO CLIENTE E DA TELA
                    ...
                    st.markdown(f"""
                        ...
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; background: #111; border: 2px solid #FFC107; border-radius: 10px; padding: 20px; margin-top: 15px; margin-bottom: 40px; text-align: center;">
                            <p style="color: #FFC107; font-family: monospace; font-size: 18px; margin-bottom: 10px; font-weight: bold;">📱 ESCANEIE PARA PEDIR UMA MÚSICA:</p>
                            <img src="{qr_url_cliente}" width="160" style="border-radius: 6px; border: 4px solid #fff; margin-bottom: 15px;" />
                            
                            <!-- LINKS AUMENTADOS E EM NEGRITO -->
                            <div style="margin-top: 10px; width: 100%; text-align: left; font-family: monospace;">
                                <p style="color: #FFC107; font-size: 16px; font-weight: bold; margin: 8px 0;">🔗 Link do Cliente:</p>
                                <a href="{link_cliente_absoluto}" target="_blank" style="color: #4CAF50; font-size: 16px; font-weight: 900; word-break: break-all; text-decoration: underline;">{link_cliente_absoluto}</a>
                                
                                <p style="color: #FFC107; font-size: 16px; font-weight: bold; margin: 12px 0 8px 0;">📺 Link da Tela (TV):</p>
                                <a href="{link_tela_absoluto}" target="_blank" style="color: #2196F3; font-size: 16px; font-weight: 900; word-break: break-all; text-decoration: underline;">{link_tela_absoluto}</a>
                            </div>

                            <div class="mic-rotating" style="font-size: 55px; margin-top: 15px;">🎤</div>
                        </div>
                    """, unsafe_allow_html=True)
