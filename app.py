...
def custom_show_register_page():
    ...
    if "token_pendente_prestador" in st.session_state and st.session_state["token_pendente_prestador"]:
        ...
        # Se foi bloqueado permanentemente
        if bloqueado:
            st.markdown(f"""
                <div style="text-align: center; padding: 40px; font-family: monospace;">
                    <h1 style="color: #ff3333; font-size: 38px; margin-bottom: 20px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">PERFIL BLOQUEADO</h1>
                    <p style="color: #ffffff; font-size: 20px; font-weight: bold; margin-bottom: 30px; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">O seu acesso foi bloqueado permanentemente após exceder o limite de recusas.</p>
                    <p style="color: #FFC107; font-size: 16px;">Contacte a administração do Grupo FF Karaoke para assistência.</p>
                </div>
            """, unsafe_allow_html=True)
            return
        ...
        if recusado:
            st.markdown(f"""
                <div style="text-align: center; padding: 40px; font-family: monospace;">
                    ...
                    <p style="color: #ffffff; font-size: 20px; font-weight: bold; margin-bottom: 10px; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">Lamentamos, mas o seu pedido de acesso foi recusado pelo Administrador.</p>
                    <p style="color: #FFC107; font-size: 14px; margin-bottom: 30px;">⚠️ Atenção: Se acumular 3 recusas e tentar novamente, o seu perfil será bloqueado.</p>
                </div>
            """, unsafe_allow_html=True)
            ...
            if not nome or not telefone:
                ...
                try:
                    ...
                    if not df_all.empty and 'phone' in df_all.columns:
                        matched = df_all[df_all['phone'] == telefone]
                        if not matched.empty:
                            for _, r in matched.iterrows():
                                rc = int(r.get('rejection_count', 0))
                                if rc > rejection_count:
                                    rejection_count = rc
                except Exception:
                    pass
                ...
                try:
                    ...
                    try:
                        ...
                        requests.put(f"https://ffkaraoke-default-rtdb.firebaseio.com/providers/{token_gerado}.json", json=dados_reg, timeout=10)
                        st.session_state["token_pendente_prestador"] = token_gerado
                        st.session_state["nome_pendente_prestador"] = nome_completo
                        st.rerun()
...
def show_provider_panel_custom(provider_token):
    ...
    # --- NOVA LÓGICA: VERIFICAR REFORÇOS RECUSADOS ---
    bloqueado = False
    contagem_recusados = 0
    try:
        res_recusados = requests.get(f"{FIREBASE_URL}/reforcos_recusados/{provider_token}.json", timeout=5)
        if res_recusados.status_code == 200 and res_recusados.json():
            dados_recusados = res_recusados.json()
            contagem_recusados = len(dados_recusados)
            if contagem_recusados >= 3: # Limite de 3 recusas para bloquear
                bloqueado = True
    except:
        pass

    if bloqueado:
        st.error("⚠️ CONTA BLOQUEADA: Demasiados pedidos de reforço inválidos ou recusados. Contacte o administrador.")
        return # Impede que o resto do painel seja carregado
    ...
    # ... [O restante do seu código de cálculo de tempo permanece igual] ...
    segundos_bónus = 0
    try:
    ...
    segundos_base = 10800 if "3 Horas" in tempo_plano else (14400 if "4 Horas" in tempo_plano else 7200)
    ...
    # --- EXIBIÇÃO DO AVISO DE RECUSAS (OPCIONAL) ---
    if contagem_recusados > 0:
        st.warning(f"⚠️ Atenção: Tens {contagem_recusados} pedidos de reforço recusados. Ao atingir 3, serás bloqueado.")

    # ... [Resto do código HTML e renderização do painel abaixo] ...
...
def show_admin_panel():
    ...
    if pendentes_count > 0:
        st.markdown(f"<div style='margin-bottom: 5px;'>⏳ <span class='badge-pendente-global'>{pendentes_count}</span></div>", unsafe_allow_html=True)
        ...
        if df_all.empty:
            ...
            if pendentes.empty:
                ...
                for index, row in pendentes.iterrows():
                    ...
                    token = row.get('token', '')
                    rejection_count = int(row.get('rejection_count', 0))
                    ...
                    with rc1:
                        st.markdown(f"<span style='font-size: 11px; color: #aaa;'>Nome</span><br>🎤 <b>{nome}</b><br><span style='font-size:10px; color: #ff9999;'>Recusas prévias: {rejection_count}</span>", unsafe_allow_html=True)
                    with rc2:
                    ...
                    with rc5:
                        ...
                        if st.button("❌ Recusar", key=f"btn_rec_{token}"):
                            try:
                                new_rc = rejection_count + 1
                                novo_estado = -2 if new_rc >= 4 else -1
                                dados_payload = {
                                    "approved": novo_estado,
                                    "rejection_count": new_rc,
                                    "rejection_reason": "O seu registo foi recusado pelo Administrador."
                                }
                                requests.patch(f"{FIREBASE_URL}/providers/{token}.json", json=dados_payload, timeout=10)
                                
                                st.warning(f"Registo de {nome} recusado (Recusa #{new_rc}). Notificação enviada.")
                                st.rerun()
                            except Exception as e:
    ...
    with aba4:
        ...
        st.write("Registo integral de todas as transações, valores e tempos solicitados (incluindo licenças expiradas, recusadas e bloqueadas).")
...
def main():
    try:
        ...
        if token:
            st.success(f"Bem-vindo ao Painel do Prestador! Token: {token}")
            return
