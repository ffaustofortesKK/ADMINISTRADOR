...
# URL do Firebase (Defina conforme a sua configuração)
FIREBASE_URL = "https://grupoffkaraoke-default-rtdb.firebaseio.com"

# Importar o módulo do prestador (criado separadamente)
try:
    from modules.prestador import renderizar_gestao_fila_prestador, obter_video_fundo
except ImportError:
    # Funções de fallback caso o módulo ainda não esteja carregado
    def renderizar_gestao_fila_prestador(token):
        st.warning("Módulo de gestão do prestador não encontrado.")
    def obter_video_fundo(token):
        return ""

def limpar_nome_musica(titulo):
    return titulo.replace(".mp4", "").replace(".mkv", "").replace("_", " ")
...
def custom_show_register_page():
    ...
    st.info("Preencha os dados para se registar como prestador.")

def show_client_page():
    st.title("Registo de Música - Cliente")
    query_params = st.query_params
    prestador = query_params.get("prestador")
    st.info(f"Bem-vindo ao painel do prestador: {prestador}")
...
def show_admin_panel():
    ...
    st.write("Gestão de prestadores e sistema.")

def show_provider_panel_custom(provider_token):
    # Simulação de dados do prestador para o painel
    nome_prestador = "Prestador Exemplo"
