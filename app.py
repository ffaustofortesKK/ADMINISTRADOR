...
# URL do Firebase (Base de dados em tempo real)
FIREBASE_URL = "https://ffkaraokecloud-default-rtdb.firebaseio.com"

# Funções auxiliares fictícias / de suporte necessárias para o fluxo
...
def limpar_nome_musica(titulo):
    return titulo.replace(".mp4", "").replace(".avi", "").strip()
...
def obter_video_fundo(provider_token):
    try:
        res = requests.get(f"{FIREBASE_URL}/config_fundo/{provider_token}.json", timeout=5)
        if res.status_code == 200 and res.json():
