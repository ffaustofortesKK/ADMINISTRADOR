        ...
        if tocando_agora:
            ...
            if url_clipe_fundo:
                ...
                <div style="width: 100vw; height: 100vh; background: black; position: fixed; top: 0; left: 0;">
                    <video width="100%" height="100%" autoplay loop muted playsinline style="object-fit: contain; background: black; width: 100%; height: 100%;">
                        <source src="{url_clipe_fundo}" type="video/mp4">
                    </video>
                </div>
                {frame_styles}
                <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; z-index: 99999; background: rgba(0,0,0,0.85); border: 3px solid #FFC107; padding: 30px 50px; border-radius: 15px; box-shadow: 0 0 30px rgba(255,193,7,0.4);">
                    <div style="font-size: 55px; margin-bottom: 10px;">🎤</div>
                    <div style="font-family: monospace; font-size: 32px; color: #FFC107; font-weight: bold; margin-bottom: 10px; text-transform: uppercase;">FF KARAOKE CLOUD</div>
                    <div style="font-family: monospace; font-size: 20px; color: #ffffff; margin-bottom: 20px;">Aguardando o próximo cantor...</div>
                    {"<div style='font-family: monospace; font-size: 18px; color: #4CAF50; background: #111; padding: 10px; border-radius: 8px; border: 1px dashed #4CAF50;'>Próximo da Fila: <b>" + limpar_nome_musica(proximo_cantor.get('musica', {})) + "</b> (" + proximo_cantor.get('cliente', 'Convidado') + ")</div>" if proximo_cantor else ""}
                </div>
                """
                components.html(fundo_html, height=750, scrolling=False)
            else:
                qr_code_html = f"""
                <style>
                    body, html {{
                        margin: 0; padding: 0; background: #000000; overflow: hidden; width: 100vw; height: 100vh;
                    }}
                    .tv-container {{
                        display: flex;
                        width: 100vw;
                ...
                <div class="tv-container">
                    <div class="left-pane">
                        ...
                        <h1 style="color: #FFC107; font-size: 32px; margin: 0 0 10px 0; font-weight: bold; text-shadow: 0 0 10px rgba(255,193,7,0.5);">FF KARAOKE CLOUD</h1>
                        <p style="color: #cccccc; font-size: 16px; margin-bottom: 25px;">Aponte a câmara do seu telemóvel para escanear o QR Code, escolher a sua música e cantar!</p>
                        
                        <div class="qr-box">
                            <img src="{qr_url_cliente}" width="210" height="210" alt="QR Code">
                        </div>
                        <div style="color: #4CAF50; font-size: 14px; font-weight: bold; background: #111; padding: 6px 14px; border-radius: 20px; border: 1px solid #4CAF50;">SCAN ME PARA PEDIR MÚSICA</div>
                    </div>

                    <div class="right-pane">
                        <div class="queue-box">
                            <h3 style="color: #FFC107; margin-top: 0; border-bottom: 2px solid #333; padding-bottom: 10px; font-size: 20px; text-align: center;">🎶 FILA DE ESPERA ATUAL</h3>
                """
                ...
                if pedidos_ativos:
                    for idx, p in enumerate(pedidos_ativos, start=1):
                        ...
                        qr_code_html += f"""
                            <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #222; font-size: 16px;">
                                <div><b>{idx}.</b> <span style="color: #FFC107;">{t_musica}</span> <span style="color: #aaa; font-size: 13px;">({c_nome})</span></div>
                                <div style="color: #FFC107; font-size: 12px; background: #222; padding: 3px 8px; border-radius: 4px;">Na Fila</div>
                            </div>
                        """
                else:
                    qr_code_html += """
                            <div style="text-align: center; color: #777; padding: 30px; font-size: 16px;">
                                Nenhum pedido na fila neste momento.<br>Seja o primeiro a cantar!
                            </div>
                    """
                ...
                """
                components.html(qr_code_html, height=750, scrolling=False)
                
    except Exception as e:
        st.error(f"Erro ao renderizar ecrã da TV: {e}")

def main():
    query_params = st.query_params
    page = query_params.get("page", "admin")
    prestador_token = query_params.get("prestador", "")

    if page == "admin":
        show_admin_panel()
    elif page == "register":
        show_register_page()
    elif page == "client_register":
        if prestador_token:
            show_client_page(prestador_token)
        else:
            st.error("Token do prestador não especificado para a página do cliente.")
    elif page == "provider_panel":
        if prestador_token:
            show_provider_panel_custom(prestador_token)
        else:
            st.error("Token do prestador não especificado.")
    elif page == "client_screen":
        if prestador_token:
            renderizar_ecra_tv(prestador_token)
        else:
            st.error("Token do prestador não especificado para o ecrã da TV.")
    else:
