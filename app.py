...
def get_pedidos_prestador(token):
    try:
        response = requests.get(
            f"{FIREBASE_URL}/pedidos/{token}.json?_t={time.time()}"
        )
...
def show_provider_panel():
    ...
    if not token:
        st.error(
            "Acesso não autorizado. Por favor, aceda através do link válido fornecido na sua aprovação."
        )
        return
    ...
    st.subheader("🎤 Painel do Prestador — FF Karaoke Cloud")
    st.markdown("---")
    ...
    # Invocação da Fila com atualização automática em fragmento a cada 3 segundos
    renderizar_gestao_fila_prestador_fragment(token)


@st.fragment(run_every=3)
def renderizar_gestao_fila_prestador_fragment(provider_token):
    st.subheader("🎵 Gestão de Pedidos dos Clientes em Tempo Real")

    pedidos = get_pedidos_prestador(provider_token)
    if not pedidos:
    ...
    # Confirmação visual para pedidos pendentes
    if pendentes:
        st.markdown(
            """
        ...
        for p in pendentes:
            ...
            col_btn_dummy1, col_center_btn, col_btn_dummy2 = st.columns(
                [1, 1.2, 1]
            )
            with col_center_btn:
                if st.button("✅ Sim", key=f"conf_sim_{p_id}", use_container_width=True):
                    atualizar_estado_pedido(provider_token, p_id, "aprovado")
                    st.success(f"Música '{titulo_p}' enviada para a tela!")
                    st.rerun()
                if st.button("❌ Não", key=f"conf_nao_{p_id}", use_container_width=True):
                    atualizar_estado_pedido(provider_token, p_id, "terminado")
            ...
            with st.container():
                ...
                with col_acao1:
                    if not is_playing:
                        if st.button(
                            "▶️ Tocar Agora",
                            key=f"play_linha_{p_id}",
                            use_container_width=True,
                        ):
                            atualizar_estado_pedido(
                                provider_token, p_id, "aprovado"
                            )
                ...
                with col_acao2:
                    if is_playing:
                        if st.button(
                            "⏹️ Terminar Atual",
                            key=f"term_linha_{p_id}",
                            use_container_width=True,
                        ):
                            atualizar_estado_pedido(
                                provider_token, p_id, "terminado"
                            )
    ...
    st.markdown(f"""
        <div class="panel-header">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <h1 style="margin: 0; color: #ffffff; font-family: monospace; font-size: 24px; text-transform: uppercase; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">PERFIL DO PRESTADOR: {nome_prestador}</h1>
