...
# Constantes e URL do Firebase
FIREBASE_URL = "https://ffkaraoke-default-rtdb.firebaseio.com"

# Funções Auxiliares de Dados e Utilitários
    ...
    try:
        ...
        if res.status_code == 200 and res.json():
            ...
            for p in pedidos:
                ...
                titulo = musica_obj.get("titulo", str(musica_obj)) if isinstance(musica_obj, dict) else str(musica_obj)
...
# Função para Painel Personalizado do Prestador
def show_provider_panel_custom(provider_token):
    ...
    .link-title { color: #4CAF50; font-family: monospace; font-weight: bold; font-size: 14px; margin-bottom: 5px; }
    .link-title-tv { color: #2196F3; font-family: monospace; font-weight: bold; font-size: 14px; margin-bottom: 5px; }
    .link-text { color: #ffffff; font-family: monospace; font-size: 13px; text-decoration: none; word-break: break-all; }
    .link-text-tv { color: #ffffff; font-family: monospace; font-size: 13px; text-decoration: none; word-break: break-all; }
    ...
    .alert-reforco {
        ...
        font-size: 16px;
        margin-bottom: 20px;
        animation: blinkAlert 1s infinite ease-in-out;
    ...
    data_registo_str = str(datetime.now())
    
    if not df_prov.empty and 'token' in df_prov.columns:
        match = df_prov[df_prov['token'] == provider_token]
        if not match.empty:
            nome_prestador = match.iloc[0].get('nome_prestador', 'Prestador')
            tempo_plano = match.iloc[0].get('tempo_plano', '2 Horas - 12 Mil Kwanzas')
            data_registo_str = match.iloc[0].get('data_registo', str(datetime.now()))

    # Calcular tempo restante com base na duração escolhida
    horas_alocadas = 2
    ...
    try:
        dt_reg = datetime.strptime(data_registo_str, "%Y-%m-%d %H:%M:%S.%f")
    except Exception:
        try:
            dt_reg = datetime.strptime(data_registo_str, "%Y-%m-%d %H:%M:%S")
        except Exception:
            dt_reg = datetime.now()

    tempo_expiracao = dt_reg + timedelta(hours=horas_alocadas)
    segundos_restantes = int((tempo_expiracao - datetime.now()).total_seconds())
    if segundos_restantes < 0:
        segundos_restantes = 0

    h_r = segundos_restantes // 3600
    m_r = (segundos_restantes % 3600) // 60
    s_r = segundos_restantes % 60
    tempo_formatado = f"{h_r:02d}:{m_r:02d}:{s_r:02d}"

    aviso_reforço_html = ""
    if segundos_restantes <= 1800:
        aviso_reforço_html = """
        <div class="alert-reforco">
            ⚠️ O SEU TEMPO ESTA TERMINANDO. PARA QUE NÃO PERCAS OS SEUS REGISTOS PEÇA REFORÇO DE TEMPO.
        </div>
        """

    url_logotipo = "https://i.imgur.com/7A9341P.png"
    st.markdown(f"""
    ...
    # Secção de Pesquisa de Vídeo Clipe de Fundo
    st.markdown("### 🎬 Pesquisar Vídeo Clipe")
    with st.form("form_fundo_prestador"):
        termo_pesquisa = st.text_input("Nome da música ou artista para o vídeo clipe de fundo:")
        btn_pesq = st.form_submit_button("Pesquisar Vídeo Clipe", type="primary")
        if btn_pesq:
            if termo_pesquisa:
                # URL padrão de vídeo de fundo para demonstração/sistema
                video_url_demo = "https://res.cloudinary.com/demo/video/upload/dog.mp4"
                try:
                    requests.put(f"{FIREBASE_URL}/fundo_prestador/{provider_token}.json", json={"url_video": video_url_demo}, timeout=5)
                    st.success(f"Vídeo clipe de fundo atualizado para: {termo_pesquisa}")
                except Exception as e:
                    st.error(f"Erro ao salvar vídeo clipe: {e}")
            else:
                st.warning("Insira um termo para pesquisar.")
                ...
                if url_clipe_fundo:
                    ...
                    st.markdown("""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 100px 20px; text-align: center; background: rgba(0,0,0,0.95); color: #ffffff; font-family: monospace; margin-top: 5px; margin-bottom: 40px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
                            ...
                            <p style="color: #ffffff; font-size: 16px; margin: 0; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">Aguardando o prestador pesquisar um vídeo clipe no painel de controle...</p>
                        </div>
                    """, unsafe_allow_html=True)
...
def show_admin_panel():
    st.title("🛠️ Painel de Administração - FFKaraoke")
    
    # Abas ou secções do ADM
    tab1, tab2 = st.tabs(["Gestão de Prestadores", "Gestão de Reforços"])
    ...
    with tab1:
        ...
        if not df_p.empty:
            for idx, row in df_p.iterrows():
                p_token = row.get('token')
                p_nome = row.get('nome_prestador', 'Desconhecido')
                p_plano = row.get('tempo_plano', 'N/A')
                p_app = row.get('approved', 1)
                
                cols = st.columns([3, 2, 1])
                with cols[0]:
                    st.write(f"**Nome:** {p_nome} | **Plano:** {p_plano}")
                with cols[1]:
                    st.write(f"**Estado:** {'Aprovado' if p_app == 1 else 'Pendente'}")
                with cols[2]:
                    if p_app == 0:
                        if st.button("Aprovar", key=f"app_adm_{p_token}"):
                            requests.put(f"{FIREBASE_URL}/prestadores/{p_token}/approved.json", json=1, timeout=5)
                            st.rerun()
        else:
            st.info("Nenhum prestador registado.")

    with tab2:
        st.subheader("Gestão de Reforços de Tempo")
        try:
            ...
            if res_ref.status_code == 200 and res_ref.json():
                ...
                for token_p, dados in reforcos.items():
                    if dados.get("approved", 0) == 0:
                        st.write(f"**Prestador:** {dados.get('nome_prestador')} | **Plano Solicitado:** {dados.get('tempo_plano')}")
                        c1, c2 = st.columns(2)
                        with c1:
                            if st.button("Sim (Aprovar)", key=f"sim_ref_{token_p}"):
                                # Atualizar o tempo do prestador somando ou redefinindo a data de registo atual
                                nova_data = str(datetime.now())
                                requests.put(f"{FIREBASE_URL}/prestadores/{token_p}/tempo_plano.json", json=dados.get('tempo_plano'), timeout=5)
                                requests.put(f"{FIREBASE_URL}/prestadores/{token_p}/data_registo.json", json=nova_data, timeout=5)
                                # Remover o pedido de reforço pendente
                                requests.delete(f"{FIREBASE_URL}/reforcos_pendentes/{token_p}.json", timeout=5)
...
def main():
    try:
        ...
        if "page" in query_params and query_params["page"] == "client_register":
            # Página de registo de músicas para clientes
            st.title("🎤 FFKaraoke - Registo de Música")
            token_prestador = query_params.get("prestador")
            if not token_prestador:
                st.error("Prestador não especificado.")
                return
            
            with st.form("form_registo_musica_cliente"):
                nome_cliente = st.text_input("Seu Nome / Convidado")
                titulo_musica = st.text_input("Título da Música Pretendida")
                btn_sub_cli = st.form_submit_button("Submeter Música")
                if btn_sub_cli:
                    if nome_cliente and titulo_musica:
                        novo_pedido = {
                            "cliente": nome_cliente,
                            "musica": {"titulo": titulo_musica, "url_cloudinary": "https://res.cloudinary.com/demo/video/upload/dog.mp4"},
                            "estado": "pendente",
                            "timestamp": time.time()
                        }
                        try:
                            requests.post(f"{FIREBASE_URL}/pedidos/{token_prestador}.json", json=novo_pedido, timeout=10)
                            st.success("Música submetida com sucesso para a fila!")
                        except Exception as err:
