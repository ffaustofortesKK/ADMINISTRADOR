...
def show_provider_panel_custom(token):
    st.write(f"Painel do Prestador - Token: {token}")

def show_admin_panel():
    st.write("Painel Administrativo Geral")
...
def renderizar_ecra_tv(provider_token):
    try:
        ...
        dados = res.json() if res.status_code == 200 and res.json() else {}
        ...
        if tocando_agora:
            ...
            c_atual = tocando_agora.get("cliente", "Convidado")
            m_atual = tocando_agora.get("musica", "Música")
            
            video_html = f"""
            <div id="countdown-screen" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: black; z-index: 99999; display: flex; flex-direction: column; justify-content: center; align-items: center; color: #FFC107; font-family: monospace; font-size: 120px; font-weight: bold; text-shadow: 2px 2px 10px rgba(255,193,7,0.5);">
                3
            </div>
            
            <div id="karaoke-container" style="display: none; width: 100vw; height: 100vh; background: black; position: fixed; top: 0; left: 0; z-index: 9999; overflow: hidden;">
                <div style="position: absolute; top: 15px; left: 20px; z-index: 10000; color: white; font-family: monospace; text-shadow: 2px 2px 4px rgba(0,0,0,0.9);">
                    <h2 style="margin: 0; color: #FFC107; font-size: 24px;">🎤 A CANTAR AGORA</h2>
                    <p style="margin: 5px 0 0 0; font-size: 20px;"><b>Cantor:</b> {c_atual} | <b>Música:</b> {m_atual}</p>
                </div>

                <div style="position: absolute; top: 15px; right: 20px; z-index: 10000;">
                    <button onclick="stopKaraoke()" style="background: #dc3545; color: white; border: none; padding: 10px 20px; font-family: monospace; font-size: 16px; font-weight: bold; border-radius: 5px; cursor: pointer; box-shadow: 2px 2px 5px rgba(0,0,0,0.5);">⏹ Parar / Próximo</button>
                </div>

                <div style="width: 100%; height: 100%; display: flex; justify-content: center; align-items: center; background: black;">
                    <video id="karaoke-player" width="100%" height="100%" autoplay playsinline controlslist="nodownload noremoteplayback" disablepictureinpicture style="object-fit: contain; background: black;">
                        <source src="{url_video}" type="video/mp4">
                        O seu navegador não suporta vídeo.
                    </video>
                </div>
                
                <div id="audio-warning" style="display: none; position: absolute; bottom: 30px; right: 30px; background: rgba(0,0,0,0.8); border: 2px solid #FFC107; padding: 12px 20px; border-radius: 8px; cursor: pointer; z-index: 10001;" onclick="unmuteVideo()">
                    <span style="color: white; font-family: monospace; font-size: 16px;">🔊 Clique aqui para ativar o som</span>
                </div>
            </div>
            ...
            with col_esq:
                if proximo_cantor:
                    ...
                    st.markdown(f"""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 15px; background: transparent; margin-bottom: 15px; display: flex; align-items: center; gap: 15px;">
                    ...
                    st.markdown("""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 15px; text-align: center; background: transparent; margin-bottom: 15px;">
                ...
                for idx, p_item in enumerate(demais_pedidos, start=2):
                    ...
                    html_caixas += f'<div style="background: transparent; border: 4px solid #FFC107; border-radius: 8px; padding: 12px; color: #ffffff; font-family: monospace; font-size: 16px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">{texto_caixa}</div>'
            ...
            with col_dir:
                if url_clipe_fundo:
                    video_fundo_html = f"""
                    <div style="display: flex; justify-content: center; background: transparent; border: 4px solid #FFC107; border-radius: 10px; padding: 5px; width: 100%; position: relative; margin-top: 5px; margin-bottom: 40px;">
                    ...
                    st.markdown("""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 100px 20px; text-align: center; background: transparent; color: #ffffff; font-family: monospace; margin-top: 5px; margin-bottom: 40px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
