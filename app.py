    ...
    try:
        ...
        if pedidos_ativos:
            ...
            for idx, p in enumerate(pedidos_ativos, start=1):
                ...
                cor_fundo_linha = "rgba(76, 175, 80, 0.2)" if is_playing else "transparent"
                acao_txt = "▶️ A Tocar" if is_playing else "⏳ Na Fila"
                
                linhas_tabela += f'<tr style="border-bottom: 1px solid #333; background: {cor_fundo_linha};"><td style="padding: 8px; color: #ffffff; text-align: center; font-weight: bold; border-right: 1px solid #333;">{idx}</td><td style="padding: 8px; color: #FFC107; font-weight: bold; border-right: 1px solid #333;">{cliente_nome.upper()}</td><td style="padding: 8px; color: #ffffff; font-weight: bold; border-right: 1px solid #333;">{titulo_musica}</td><td style="padding: 6px; text-align: center; color: #FFC107; font-weight: bold;">{acao_txt}</td></tr>'

            tabela_html = f'<div style="overflow-x: auto; margin-bottom: 15px;"><table style="width: 100%; border-collapse: collapse; background: #000000; border: 2px solid #FFC107; font-family: monospace;"><thead><tr style="border-bottom: 2px solid #FFC107; background: rgba(255,193,7,0.1);"><th style="padding: 8px; color: #FFC107; text-align: center; width: 50px; border-right: 1px solid #FFC107;">Nº</th><th style="padding: 8px; color: #FFC107; text-align: left; width: 150px; border-right: 1px solid #FFC107;">CLIENTE</th><th style="padding: 8px; color: #FFC107; text-align: left; border-right: 1px solid #FFC107;">TITULO</th><th style="padding: 8px; color: #FFC107; text-align: center; width: 100px;">AÇÃO</th></tr></thead><tbody>{linhas_tabela}</tbody></table></div>'
                ...
                if is_playing:
                    st.markdown(f"""
                        <div style="background: #000000; border: 3px solid #FFC107; border-radius: 8px; padding: 15px; text-align: center; margin-bottom: 15px;">
                            ...
                            <div style="color: #ffffff; font-family: monospace; font-size: 16px; font-weight: bold;">
                                🎶 {titulo_musica}
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    with c1:
                        if st.button("▶️ Tocar o Karaoke", key=f"tocar_{p.get('id')}", use_container_width=True):
                            terminar_todas_musicas_ativas(provider_token, pedidos)
                            atualizar_estado_pedido(provider_token, p.get('id'), 'aprovado')
                            st.rerun()
                    with c2:
                        if st.button("⏹️ Parar o Karaoke", key=f"parar_{p.get('id')}", use_container_width=True):
                            terminar_todas_musicas_ativas(provider_token, pedidos)
                            st.rerun()
                    with c3:
                    ...
                    col_txt, col_rem = st.columns([4, 1])
                    with col_txt:
                        st.markdown(f"<span style='font-family: monospace; font-size: 12px; color: #888;'>Fila #{idx} - {cliente_nome}: {titulo_musica}</span>", unsafe_allow_html=True)
                    with col_rem:
                        if st.button("❌ Remover", key=f"rem_{p.get('id')}", use_container_width=True):
                            atualizar_estado_pedido(provider_token, p.get('id'), 'terminado')
                            st.rerun()
        else:
            st.markdown("""
