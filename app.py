...
def renderizar_gestao_fila_prestador(provider_token):
    try:
        ...
        tocando_agora = next(
            (p for p in pedidos_ativos if p.get("estado") == "aprovado"), None
        )
        ...
        with col_esq:
            ...
            if pedidos_ativos:
                for idx, p in enumerate(pedidos_ativos, start=1):
                    ...
                    c_num, c_cli, c_tit, c_btn = st.columns(
                        [0.5, 2, 4, 0.8]
                    )
                    with c_num:
                        st.markdown(
                            f"<div style='background:#000; color:#FFC107; border:1px solid #FFC107; padding:6px; text-align:center; font-family:monospace; font-weight:bold; border-radius:4px;'>{idx}</div>",
                            unsafe_allow_html=True,
                        )
                    with c_cli:
                        st.markdown(
                            f"<div style='background:#000; color:#FFC107; border:1px solid #FFC107; padding:6px; font-family:monospace; font-weight:bold; border-radius:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;'>{cliente_nome}</div>",
                            unsafe_allow_html=True,
                        )
                    with c_tit:
                        st.markdown(
                            f"<div style='background:#000; color:#FFC107; border:1px solid #FFC107; padding:6px; font-family:monospace; font-weight:bold; border-radius:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;'>{titulo_musica}</div>",
                            unsafe_allow_html=True,
                        )
                    with c_btn:
                        if st.button(
                            "✕",
                            key=f"del_fila_{p.get('id')}",
                            use_container_width=True,
                        ):
                            atualizar_estado_pedido(
                                provider_token, p.get("id"), "terminado"
                            )
                            st.rerun()
                st.markdown(
            ...
            if tocando_agora:
                cantor_atual = tocando_agora.get(
                    "cliente", "CONVIDADO"
                ).upper()
                musica_atual = limpar_nome_musica(
                    tocando_agora.get("musica", {})
                )
                ...
                with c_t1:
                    if st.button(
                        "▶️ Tocar o Karaoke",
                        key=f"btn_tocar_{tocando_agora.get('id')}",
                        use_container_width=True,
                    ):
                        terminar_todas_musicas_ativas(provider_token, pedidos)
                        atualizar_estado_pedido(
                            provider_token, tocando_agora.get("id"), "aprovado"
                        )
                        st.rerun()
                with c_t2:
                    if st.button(
                        "⏹️ Parar o Karaoke",
                        key=f"btn_parar_{tocando_agora.get('id')}",
                        use_container_width=True,
                    ):
                        terminar_todas_musicas_ativas(provider_token, pedidos)
                        st.rerun()
                with c_t3:
                    if st.button(
                        "⏭️ Avançar Karaoke",
                        key=f"btn_prox_{tocando_agora.get('id')}",
                        use_container_width=True,
                    ):
                        atualizar_estado_pedido(
                            provider_token, tocando_agora.get("id"), "terminado"
                        )
                        restantes = [
                            x
                            for x in pedidos_ativos
                            if x.get("id") != tocando_agora.get("id")
                        ]
                        if restantes:
                            atualizar_estado_pedido(
                                provider_token,
        ...
        with col_dir:
            ...
            for idx, label in enumerate(opcoes_labels):
                if label != "Nenhum (Ecrã Preto)":
                    ...
                    if video_fundo_atual and (
                        video_fundo_atual in url_mapeada
                        or url_mapeada in video_fundo_atual
                    ):
                        index_atual = idx
                        break
            ...
            with st.form(key="form_video_fundo_pos"):
                st.markdown(
                    "<div style='font-family: monospace; color: #ffffff; font-size: 13px; font-weight: bold; margin-bottom: 5px;'>Pesquisar Vídeo Clipe</div>",
                    unsafe_allow_html=True,
                )
                ...
                btn_salvar_fundo = st.form_submit_button(
                    "🎬 TOCAR VIDEO CLIP", use_container_width=True
                )
                if btn_salvar_fundo:
                    valor_a_guardar = (
                        ""
                        if escolha_video == "Nenhum (Ecrã Preto)"
                        else mapa_url_por_label.get(escolha_video, "")
                    )
                    definir_video_fundo(provider_token, valor_a_guardar)
                    st.success(
                        "Vídeo clipe de fundo atualizado e em reprodução na tela!"
                    )
                    st.rerun()
...
def show_provider_panel_custom(provider_token):
    url_logotipo = (
        "https://cdn.phototourl.com/free/2026-08-03-8b13edf5-0257-491d-ab78-f0d5329ffc15.jpg"
    )
    url_fundo_painel = (
        "https://cdn.phototourl.com/free/2026-08-03-694a4a2e-9914-4da8-93b2-87538a4805ab.png"
    )
    ...
    if not df_prov.empty and "token" in df_prov.columns:
        ...
        if not match.empty:
            row = match.iloc[0]
            nome_prestador = row.get(
                "nome_prestador", row.get("nome", "CARLOS MIGUEL")
            ).upper()
            tempo_plano = row.get(
                "tempo_plano", row.get("tempo", "2 Horas - 12 Mil Kwanzas")
            )
    ...
    try:
        res_ref = requests.get(
            f"{FIREBASE_URL}/reforcos_aprovados/{provider_token}.json",
            timeout=5,
        )
    ...
    if data_registo_str:
        try:
            ...
            try:
                ...
                dt_reg = datetime.fromisoformat(
                    data_registo_str.replace("Z", "+00:00").split("+")[0]
                )
    ...
    with col_topo_3:
        st.markdown(
            f'<div style="text-align: right;"><img src="{url_logotipo}" class="top-logo" /></div>',
            unsafe_allow_html=True,
        )
    ...
    with col_qr:
        st.markdown(
            "<div style='font-family: monospace; color: #ffffff; font-size: 11px; font-weight: bold; margin-bottom: 3px; text-align: center;'>QR CODE CLIENTE</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"""
    ...
    if segundos_restantes <= 1800:
        ...
        with st.form("form_reforco_prestador"):
            referencia_comprovativo = st.text_input(
            ...
            if btn_sub_reforco:
                if not referencia_comprovativo:
                    st.error(
                        "Por favor, preencha a Referência de Pagamento / Nº de Comprovativo."
                    )
                else:
                    dados_reforco = {
                    ...
                    try:
                        ...
                        requests.put(
                            f"{FIREBASE_URL}/reforcos_pendentes/{provider_token}/{ref_id}.json",
                            json=dados_reforco,
                            timeout=10,
                        )
                        st.success(
                ...
                if proximo_cantor:
                    ...
                    st.markdown("""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 15px; text-align: center; background: rgba(0,0,0,0.95); margin-bottom: 15px;">
                            <h2 style="color: #ffffff; margin: 0; font-family: monospace; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">🎤 FILA DE ESPERA</h2>
                        </div>
                    """, unsafe_allow_html=True)
                ...
                if url_clipe_fundo:
                    ...
                    st.markdown("""
                        <div style="border: 4px solid #FFC107; border-radius: 10px; padding: 100px 20px; text-align: center; background: rgba(0,0,0,0.95); color: #ffffff; font-family: monospace; margin-top: 5px; margin-bottom: 40px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">
                            ...
                            <p style="color: #ffffff; font-size: 16px; margin: 0; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.9);">Vídeo clipe ...</p>
                        </div>
                    """, unsafe_allow_html=True)
