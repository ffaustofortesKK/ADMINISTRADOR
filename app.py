...
def limpar_nome_musica(titulo):
    if not titulo:
        return "Karaoke"
    return str(titulo).replace(".mp4", "").replace(".avi", "").replace(".mkv", "").replace("_", " ")
    ...
    try:
        ...
        if res.status_code == 200 and res.json():
            ...
            if isinstance(data, dict):
                ...
                return pd.DataFrame([r for r in data if r is not None])
    except Exception:
        pass
    return pd.DataFrame()
    ...
    try:
        ...
        if response.status_code == 200 and response.json():
            ...
            for p in pedidos:
                ...
                titulo = musica_info.get("titulo", "Música") if isinstance(musica_info, dict) else str(musica_info)
                estado = p.get("estado", "pendente")
                
                col1, col2, col3 = st.columns([3, 2, 2])
                with col1:
                    st.markdown(f"**Cliente:** {cliente} <br>🎵 **Música:** {titulo} <br>Estado: `{estado}`", unsafe_allow_html=True)
                with col2:
                    if estado == "pendente":
                ...
                with col3:
                    if estado != "terminado":
                        if st.button("Terminar / Remover", key=f"term_{p_id}"):
...
def show_provider_panel_custom(provider_token):
    st.markdown("""
        ...
        .link-title { font-family: monospace; color: #FFC107; font-weight: bold; font-size: 14px; }
        .link-title-tv { font-family: monospace; color: #4CAF50; font-weight: bold; font-size: 14px; }
        .link-text { font-family: monospace; color: #ffffff; word-break: break-all; }
        .link-text-tv { font-family: monospace; color: #ffffff; word-break: break-all; }
        .qr-box {
            background: #ffffff;
            padding: 10px;
            border-radius: 8px;
    ...
    # Dados simulados/padrão para exibição do painel
        ...
        <div class="panel-header">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    ...
                    <p style="margin: 3px 0 0 0; color: #ffffff; font-size: 13px; font-family: monospace; font-weight: bold;">TOKEN: <code style="background: #222; color: #ffffff; padding: 2px 6px; border-radius: 4px;">{provider_token}</code></p>
                </div>
            </div>
            <div style="background: rgba(255,193,7,0.15); border: 2px solid #FFC107; padding: 6px 12px; border-radius: 8px; text-align: right;">
                ...
                <div style="font-family: monospace; color: #ffffff; font-size: 15px; font-weight: bold;">⏱️ {tempo_formatado} ({tempo_plano})</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
        ...
        if tocando_agora:
            ...
            video_html = f"""
                ...
                <script>
                    ...
                    var video = document.getElementById('karaoke-player');
                    if (video) {{
                        video.onended = function() {{ stopKaraoke(); }};
                    }}
                </script>
            """
...
def show_client_page():
    st.markdown("### 📝 Registo de Música para Cliente")
    st.info("Página de registo de cliente ativa.")

def custom_show_register_page():
    st.markdown("### 📋 Registo de Novo Prestador")
    st.info("Formulário de registo ativo.")

def show_admin_panel():
    st.markdown("### 🛠️ Painel de Administração")
    st.info("Painel de administração carregado com sucesso.")
...
def main():
    try:
        ...
        st.markdown("""
            ...
            h1, h2, h3, h4, h5, h6, p, span, label, div, button, input { font-weight: bold !important; }
            </style>
        """, unsafe_allow_html=True)
